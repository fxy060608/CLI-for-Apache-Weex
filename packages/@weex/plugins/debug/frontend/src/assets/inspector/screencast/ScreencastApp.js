...
Screencast.ScreencastApp = class {
  constructor() {
    // @overwrite for support customer debugger
    // change default value of screencastEnabled
    if (window.parent) {
      let search = window.parent.location.search.slice(1);
      let params = search.split('&');
      let query = {};
      for (let index in params) {
        let q = params[index].split('=');
        query[q[0]] = q[1] 
      }
      if (query['type'] !== 'weex') {
